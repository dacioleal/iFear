//
//  Constants.m
//  iFear
//
//  Created by José Alberto Martín Falcón on 26/03/14.
//  Copyright (c) 2014 Dacio Leal Rodriguez. All rights reserved.
//

#import "Constants.h"

@implementation Constants

NSString * const ANIMACION = @"Animacion";
NSString * const CATASTROFE = @"Catástrofes";
NSString * const CORTOMETRAJE = @"Cortometraje";
NSString * const EXORCISMO = @"Exorcismos";
NSString * const JHORROR = @"J-Horror";
NSString * const MONSTRUOS = @"Monstruos";
NSString * const ASESINOSSERIE = @"Asesinos en serie";
NSString * const SCIFI = @"Ciencia-ficción";
NSString * const DOCUMENTAL = @"Documental";
NSString * const FAKE = @"Falso documental";
NSString * const LICANTROPOS = @"Licántropos";
NSString * const SECTAS = @"Sectas";
NSString * const VAMPIROS = @"Vampiros";
NSString * const HECHOSREALES = @"Basada en hechos reales";
NSString * const COMEDIA = @"Comedia de terror//Parodia";
NSString * const ENFERMEDADES = @"Enfermedades";
NSString * const FANTASMAS = @"Fantasmas";
NSString * const MANICOMIOS = @"Manicomios";
NSString * const SECUELA = @"Secuela//Precuela";
NSString * const ZOMBIES = @"Zombies";
NSString * const BRUJERIA = @"Brujería";
NSString * const CASASENCANTADAS = @"Casas encantadas";
NSString * const EXTRATERRESTRES = @"Extraterrestres";
NSString * const GIALLO = @"Giallo";
NSString * const METRAJE = @"Metraje encontrado";
NSString * const SERIETV = @"Serie TV";

@end
